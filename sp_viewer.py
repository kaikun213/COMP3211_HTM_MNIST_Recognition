...
class SPViewer(object):
  ...
  def __init__(self,
    ...
    self.imageListPath = imageListPath
  ...
  def run(self):
    ...
    # Read image file names from XML file and populate imagePatches with images
    xmldoc = minidom.parse(self.imageListPath)
    directories = self.imageListPath.split("/")
    directories.pop()
    directoryPath = ''
    ...
    for imageFile in fileList:
      filename = imageFile.attributes['file'].value
      imagePatches.append(Image.open(directoryPath + filename))
      #imagePatches[-1].show()

    # Convert those to bit vectors for input into CLA
    vectorPatches = [self._convertToVector(patch) for patch in imagePatches]
    ...
    for i in range(1, self.epochCount + 1):
      ...
      for j, patch in enumerate(vectorPatches):
        ...
        # Convert activeArray to hex and print
        value = 0
        for j,num in enumerate(activeArray):
          value = value << 1
          value += int(num)
          #print int(num), hexValue
        print '%6s' % hex(value),
